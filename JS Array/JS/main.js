...
let results = [
  ...
  //1) freight 1 dən kiçik olan sifarişləri console-a yazdır
//   const filteredOrders = results.filter(item => item.order.freight < 1);
//    console.log(filteredOrders);
  
   //2) customerID ERNSH olan neçə ədəd sifariş var?
//    const lengthOrders = results.filter(item=> item.order.customerId ==="ERNSH").length
//    console.log(lengthOrders);

  //3) employeeİd- görə sortlayın.
//   const method= results.sort((a,b) => a.order.employeeId - b.order.employeeId)
//   console.log(method);

//4) OrderDetailsi 3-dən çox olanları console-a yazdır.
//const order=results.filter(item=>item .orderDetails.length>3);
//console.log(order);

//5) İD-si 11075 olanı tap və yazdır. (find)
const id=results.find(item =>item.order.iD===11075)
console.log(id);